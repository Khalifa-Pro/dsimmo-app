            ...
            steps {
                script {
                    ...
                    withSonarQubeEnv('SonarQube') {  // Nom du serveur SonarQube configuré dans Jenkins
                        sh "${SCANNER_HOME}/bin/sonar-scanner -Dsonar.projectKey=test-dsimmo-app -Dsonar.sources=. -D.sonar.host.url=http://172.17.0.2:9000 -Dsonar.login=sqp_d5d6859a94ad7dd261a37dcdc14cbbe0a21f11b1"
                    }
                }
            }
        }
